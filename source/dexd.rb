		...
		# /url.com.{css,js,html}
			...
			when "html"
				...
				Dir.glob("{global,utilities,*.*}/*/").each do |k|
					k = k[0...-1]
					titles[k] = k.rpartition("/")[2].titleize
					authors[k] = nil
					descriptions[k] = nil
				end
				...
				Dir.glob("{global,utilities,*.*}/*/info.yaml").each do |y|
					k = y[0...-10]
					info = YAML::load_file(y)
					titles[k] = info["Title"] if info["Title"]
					authors[k] = info["Author"] if info["Author"]
					descriptions[k] = info["Description"] if info["Description"]
				end
